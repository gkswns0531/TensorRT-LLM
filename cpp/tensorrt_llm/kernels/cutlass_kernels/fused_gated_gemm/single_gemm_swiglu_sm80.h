...
// SwiGLU Activation Kernel - adapted from MoE implementation
constexpr static int SWIGLU_ACTIVATION_THREADS_PER_BLOCK = 256;

template <class ActivationOutputType, class GemmOutputType>
__global__ void swiGLUActivationKernel(ActivationOutputType* output, GemmOutputType const* gemm_result,
    ...
    int64_t m, int64_t n, float output_scale)
{
    ...
    int64_t const stride = SWIGLU_ACTIVATION_THREADS_PER_BLOCK;
...
// Single GEMM SwiGLU class - simplified from MoE approach
...
public:
    ...
    cutlass::Status run(Arguments const& args, void* workspace = nullptr, cudaStream_t stream = nullptr) {
        ...
        int64_t blocks = m;
        int64_t threads = SWIGLU_ACTIVATION_THREADS_PER_BLOCK;
        ...
        swiGLUActivationKernel<<<blocks, threads, 0, stream>>>(
